<template>
  <div class="goods-search-form-container">
    <div class="community-header">
      <Form
        ref="formItemInvestment"
        :model="formItem"
      ...
      >
        ...
        <div style="padding: 30px 10px 10px 20px;background:#fff;margin-top:20px;">
          <div style="white-space: nowrap;">
            <span class="attract-font" style="padding-top:7px;margin-right:13px;">
              楼
              <span style="display:inline-block;width:25px;"></span>层
            </span>
            ...
            <Form-item label="物理空间" class="daily-form">
              <Select v-model="formItem.binding" style="width: 200px;;margin-left:10px;" clearable>
                <Option
                  v-for="item in bindingtList"
                  :value="item.value"
                  :key="item.value"
                >{{ item.label }}</Option>
              </Select>
          ...
          <div style="white-space: nowrap; margin-right:0;">
            <div class="daily-form">
              ...
              <Form-item label="商品属性" class="daily-form">
                ...
                <Select v-model="formItem.suiteType" style="width: 90px;margin-right:0;" clearable>
                  <Option
                    v-for="item in suiteType"
                    :value="item.value"
                    :key="item.value"
                  >{{ item.label }}</Option>
                </Select>
            ...
            <Button
              type="ghost"
              style="margin-left:13px;vertical-align: top;border:solid 1px #499df1;color:#499df1;box-shadow: 0 1px 6px rgba(0, 0, 0, 0.2), 0 1px 4px rgba(0, 0, 0, 0.2);"
              @click="clearClick"
            >清除</Button>
          </div>
          <!-- 第三行-->
          <div style="white-space: nowrap;">
            ...
            <div class="daily-form">
              <span
                class="attract-font"
                style="padding-top:7px;margin-right:13px;margin-left:20px;"
              >
                面
                <span style="display:inline-block;width:25px;"></span>积
              </span>
...
export default {
  ...
  data() {
    ...
    const validateStation = (rule, value, callback) => {
      ...
      if (value && !reg.test(value)) {
        ...
        this.formItem.stationsMin &&
        this.formItem.stationsMax &&
        Number(this.formItem.stationsMin) > Number(this.formItem.stationsMax)
      ) {
        callback("后者需要大于前者");
      } else {
        callback();
    ...
    const validatePrice = (rule, value, callback) => {
      ...
      if (value && !reg.test(value)) {
        ...
        this.formItem.priceMin &&
        this.formItem.priceMax &&
        Number(this.formItem.priceMin) > Number(this.formItem.priceMax)
      ) {
        callback("后者需要大于前者");
      } else {
        callback();
    ...
    const validateDiscount = (rule, value, callback) => {
      ...
      if (value && !reg.test(value)) {
        ...
        this.formItem.discountMin &&
        this.formItem.discountMax &&
        Number(this.formItem.discountMin) > Number(this.formItem.discountMax)
      ) {
        callback("后者需要大于前者");
      } else {
        callback();
    ...
    const validatearea = (rule, value, callback) => {
      ...
      if (value && !reg.test(value)) {
        ...
        this.formItem.areaMin &&
        this.formItem.areaMax &&
        Number(this.formItem.areaMin) > Number(this.formItem.areaMax)
      ) {
        callback("后者需要大于前者");
      } else {
        callback();
    ...
    return {
      ...
      ruleInvestment: {
        ...
        stationsMin: [{ validator: validateStation, trigger: "change" }],
        stationsMax: [{ validator: validateStation, trigger: "change" }],
        priceMin: [{ validator: validatePrice, trigger: "change" }],
        priceMax: [{ validator: validatePrice, trigger: "change" }],
        orderPriceMin: [{ validator: validateOrder, trigger: "change" }],
        orderPriceMax: [{ validator: validateOrder, trigger: "change" }],
        areaMin: [{ validator: validatearea, trigger: "change" }],
        areaMax: [{ validator: validatearea, trigger: "change" }],
        rentTime: [{ validator: validateTime, trigger: "change" }]
      },
      num: 0
    };
  },
  ...
  mounted() {
    if (this.communityId) {
      this.getFloorList(this.communityId);
    }
  ...
  methods: {
    ...
    getCommunityList(id) {
      this.$http
        .get("getDailyCommunity", { cityId: id })
        .then(res => {
    ...
    getFloorList(param) {
      ...
      this.$http
        .get("getDailyFloor", { communityId: param })
        .then(res => {
